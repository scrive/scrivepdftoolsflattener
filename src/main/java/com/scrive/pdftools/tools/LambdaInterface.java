...
public class LambdaInterface {
        ...
        getS3FileName(callJSON, outputStream).ifPresent(s3fileName -> {
            getFileContent(s3fileName, outputStream).ifPresent(fileContent -> {
                ...
                try {
                    ...
                    e.printStackTrace();
                    respondWithError(outputStream, "Can't parse spec as JSON");
                    return;
                }
    ...
    public void handleRequest(InputStream inputStream, OutputStream outputStream, Context context) {
        ...
        try {
            ...
            e.printStackTrace();
            respondWithError(outputStream, "Invalid input - call spec has to be a json");
            return;
        }

        if (!callJSON.has(RQ_PARAM_ACTION)) {
            respondWithError(outputStream, "Invalid action. Action has to be set");
            return;
        }
    ...
    private static Optional<String> getS3FileName(final JSONObject callJSON, final OutputStream outputStream) {
        try {
            ...
            e.printStackTrace();
            respondWithError(outputStream, "Invalid input - can't fetch s3FileName");
    ...
    private static Optional<String> getFileContent(final String s3FileName, final OutputStream outputStream) {
        try {
            ...
            e.printStackTrace();
            respondWithError(outputStream, "Failed to fetch spec file from S3");
            return Optional.empty();
        }
    }
}
